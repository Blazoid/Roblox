...
local SaveManager = {} do
	...
	SaveManager.Parser = {
		...
		ColorPicker = {
			...
			Load = function(idx, data)
				if Flags[idx] then 
					Flags[idx]:SetValueRGB(Color3.fromHex(data.value), data.transparency)
				end
			end,
		},
		KeyPicker = {
			...
			Load = function(idx, data)
				if Flags[idx] then 
					Flags[idx]:SetValue({ data.key, data.mode })
				end
			end,
		},
		...
		Input = {
			...
			Load = function(idx, data)
				if Flags[idx] and type(data.text) == 'string' then
					Flags[idx]:SetValue(data.text)
				end
			end,
		},
	}
	...
	function SaveManager:Save(name)
		...
		for idx, toggle in next, Flags do
			if self.Ignore[idx] then continue end

			table.insert(data.objects, self.Parser[toggle.Type].Save(idx, toggle))
		end

		for idx, option in next, Flags do
	...
	function SaveManager:BuildConfigSection(tab)
		...
		local section = tab:AddGroupbox({ Name = "Configuration" })
		...
		section:AddButton('Create', function()
			...
			Flags.SaveManager_ConfigList:SetValues(self:RefreshConfigList())
